...
// Vanilla
#define WE_VANILLA_RANDOM           TRUE    // If TRUE, Pokemon can randomly spawn on tiles that can trigger wild encounters, as in vanilla.
#define WE_FLAG_NO_ENCOUNTER        0       // If this flag is set, wild encounters will be disabled.

// Double Wild Battles
#define WE_DOUBLE_WILD_CHANCE            0          // % chance of encountering two Pokemon in a Wild Encounter.
#define WE_DOUBLE_WILD_REQUIRE_2_MONS    FALSE      // If set to TRUE, Wild Double Battles will default to Single Battles when the player only has 1 usable Pokemon, ignoring WE_DOUBLE_WILD_CHANCE and WE_FLAG_FORCE_DOUBLE_WILD.
...
#define WE_SMART_WILD_AI_FLAG        0     // If this flag is set, wild Pokemon will become smart, with all AI flags enabled.
#define WE_FLAG_NO_CATCHING          0     // If this flag is set, the ability to catch wild Pokemon is disabled.
...
#define WE_OW_ENCOUNTERS                        FALSE   // If TRUE, OW Pokemon can spawn as Overworld Wild Encounters on the current map. Requires OW_POKEMON_OBJECT_EVENTS.
